...
Formerly `inference.py`; renamed to `run.py` per the organizers' final-submission
announcement (docs/decisions.md D75). `inference.py` still exists as a 3-line back-compat
shim importing from this file -- this is the file that is graded and timed.

    python run.py <degraded_dir> <restored_dir>              # positional (per the announcement)
    python run.py --input_dir <degraded> --output_dir <restored>   # flags (per the original spec)
...
#: `weights/` is this repo's own convention; `models/` is a byte-identical mirror added solely
#: to satisfy the organizers' announced submission-folder shape (docs/decisions.md D75, V70
#: keeps the two in sync). Checked in that order so nothing changes for this repo's own use.
SCRIPT_DIR = Path(__file__).resolve().parent
DEFAULT_WEIGHTS = SCRIPT_DIR / "weights" / "best.pt"
if not DEFAULT_WEIGHTS.exists():
    _models_alt = SCRIPT_DIR / "models" / "best.pt"
    if _models_alt.exists():
        DEFAULT_WEIGHTS = _models_alt
...
def build_argparser() -> argparse.ArgumentParser:
    """Two ways to supply the two mandatory directories -- positional or flags (V02).

    The organizers' final-submission announcement (docs/decisions.md D75) specifies
    `python run.py <input_dir> <output_dir>` (positional); the original spec specifies
    `--input_dir/--output_dir` (flags). Neither is required=True at the argparse level any
    more -- that would make the other form a parse-time error -- instead both are accepted
    and `main()` resolves whichever was actually supplied, erroring clearly (non-zero exit)
    if neither form gives both directories. This is enforced BEHAVIORALLY by V02, not by an
    argparse `required=True` flag, and is strictly stronger than the old static check: it
    proves both invocation styles actually work AND that omitting both still fails clearly.
    """
    ap = argparse.ArgumentParser(description="Restore degraded images (2x SR + denoise).")
    ap.add_argument("input_dir_pos", nargs="?", default=None, metavar="input_dir",
                    help="directory of degraded .npy inputs (positional form)")
    ap.add_argument("output_dir_pos", nargs="?", default=None, metavar="output_dir",
                    help="directory for restored .npy outputs (positional form)")
    ap.add_argument("--input_dir", default=None, help="directory of degraded .npy inputs")
    ap.add_argument("--output_dir", default=None, help="directory for restored .npy outputs")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not input_dir or not output_dir:
        _err("both input and output directories are required -- either "
             "'python run.py <input_dir> <output_dir>' (positional) or "
             "'python run.py --input_dir X --output_dir Y' (flags)")
        return 2
    in_dir, out_dir = Path(input_dir), Path(output_dir)
    if not in_dir.is_dir():
        _err(f"input directory is not a directory: {in_dir}")
        return 1
