...
> ## STATUS — 2026-08-15: FIRST TRAINED CHECKPOINT AVAILABLE
>
> `weights/best.pt` is present and tracked. It contains a 5x5 linear least-squares
> restoration fit learned from the 2,800-pair training partition and embedded in the configured
> NAFSR checkpoint format. `inference.py --require_weights` reloads it with strict state-dict
> validation.
>
> On the committed 400-pair validation split, saved-output evaluation measures
> **26.3277 ± 3.5051 dB PSNR**, beating the strongest classical baseline by 0.0555 dB.
> The input-only final test folder has not been used for training or metric computation. Its
> 400 restored outputs are packaged as `semicon_final_outputs.zip` with SHA256
> `17fdeba4b8d19b5b4ddbe0b3d430851b9385200c8922964b354c70ba108d5eed`.
>
...
| **Ours (trained LS-5, NAFSR checkpoint)** | **26.3277 ± 3.5051** | **0.65999 ± 0.14791** | **0.39992 ± 0.14045** | **31.8 img/s (local Mac CPU, fp32)** |
...
The final test set has no ground truth, so no final-test PSNR, SSIM, or LPIPS is reported.
The 400-file archive is a submission/release artifact rather than a Git-tracked blob; its
per-file hashes and output-contract evidence are committed under
...
If the checkpoint is removed, the default two-argument run prints this on stderr and still
exits 0:
...
That fallback is deliberate, but its output is **not** a model result. The packaged final
outputs were generated with `--require_weights`, which makes a missing or invalid checkpoint
a hard error.
...
| `results/restored_test_outputs/` | manifests and hashes for 400 final outputs; the 90,992,260-byte zip remains a release/submission asset |
...
The measured local Mac CPU run restored 400 files in **12.57 s** at **31.8 img/s**, batch 32,
fp32. This is local generation timing, not an official H100, CUDA, or competition-server
measurement. A startup-vs-compute breakdown was not separately measured, and the external
benchmark harness remains pending. Full provenance is in `results/runtime_report.md`.

## Verification

Correctness for this project is defined by `docs/VERIFICATION_CONTRACT.md` (immutable) and
executed by `scripts/verify_all.py`, which writes
...
(V27, V30, V31, V48), and validation isolation (V29). Final-output contract checks now use
the committed manifests; public release-URL verification remains pending until the archive is
uploaded, and the U-Net comparison is still unavailable. The authoritative per-check status is
