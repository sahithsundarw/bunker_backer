...
| **Ours — `r2_nb8_psnrloss` (LS-5 + 8-block NAFSR residual, current `weights/best.pt`)** | **28.0394 ± 4.1881** | **0.74804 ± 0.15275** | **0.29571 ± 0.16672** | 7.1 img/s (local Mac CPU; see `results/runtime_report.md`) |
...
The dataset lives outside the repository and is never
...
| `results/restored_test_outputs/` | mandatory model outputs, generated from the current `weights/best.pt` (28.0394 dB). **Holds a manifest and hashes, not the output bytes** — the 400-file archive (91,051,052 B, sha256 `a33a9a5a129bb006eccb5cf3367abad3456c63d96c1e7bb26e76800e7e375f98`) ships as a GitHub Release asset (`docs/decisions.md` D23); upload is the one remaining manual step, see that folder's `README.md` |
| `results/runtime_report.md` | local Mac CPU runtime measurement for the current checkpoint (400 images, 56.73 s, 7.1 img/s) — explicitly not an H100/CUDA number |
...
`results/runtime_report.md` records a **local Mac CPU** measurement for the current checkpoint:
400 images, batch size 32, fp32, 56.73 s wall-clock, 7.1 img/s, taken from `inference.py`'s own
reported timing for the run that produced `results/restored_test_outputs/`. It is explicitly
**not** an H100 or CUDA number — this repository does not have GPU access to KLA's scoring
hardware, and no such number is fabricated or presented as one.

The external-process, startup-vs-compute breakdown described by `scripts/benchmark_runtime.py`
(timing the whole process from the outside, not an internal forward-pass timer, so that the
~85–95% of wall-clock spent on startup is not hidden) is **not yet implemented** — see
`docs/STATE.md`. The number in `results/runtime_report.md` today is `inference.py`'s
self-reported wall-clock for the run, which is a real measurement but not that external
breakdown.
...
2026-08-15 pass checkpoint presence/loading (V06, V35, V59), saved-output metric evaluation
(V27, V30, V31, V48), and validation isolation (V29). Runtime-report checks remain pending,
and the U-Net comparison is still unavailable. The authoritative per-check status is
