...
> ## STATUS — 2026-08-15: BACKTRACKING SEARCH COMPLETE
>
> `weights/best.pt` is a tracked adaptive 7x7 local least-squares model with 16 intensity
> bins and 8 texture bins, fitted on the 2,800-pair training partition only.
> `inference.py --require_weights` reloads it with strict state-dict validation.
>
> On the committed 400-pair validation split, saved-output evaluation measures
> **27.4379 ± 3.7917 dB PSNR** with TTA, beating NLM by 1.1657 dB and the LS-5
> parent by 1.1102 dB. The 29.0 dB search target was **not reached**.
> The input-only final test folder has not been used for training or metric computation.
>
> Checkpoint SHA256:
> `a2c53f8667c3f63efb23cb1411bac8eb2f394ac818e6cd7a9f5352736c3990cf`.
> Search ledger: `results/backtrack_experiments.csv`.
...
| **Ours (adaptive LS, 7x7, TTA)** | **27.4379 ± 3.7917** | **0.73005 ± 0.14216** | **0.32314 ± 0.16588** | not yet measured |
...
| | |
|---|---|
| Search host OS | macOS 26.6.1, arm64 |
| Python | 3.12 |
| PyTorch | 2.13.0 |
| Accelerator available for this search | none; CUDA false, MPS unavailable |
...
The promoted checkpoint is reproduced by the complete backtracking command:

```text
python scripts/backtrack_search.py --data_root <dataset_root> \
  --out_root runs/backtrack_search --target 29.0 --max_trials 11 --device cpu
```

The runner fits one supervised local model per intensity/texture region using only the 2,800
training names, writes all 400 validation predictions to disk, reloads them for pinned
PSNR/SSIM/LPIPS evaluation, and replaces `weights/best.pt` only after a strict PSNR
improvement. The selected model uses 7x7 filters, 16 intensity bins, 8 local-standard-
deviation bins, and 8-way dihedral TTA. It has 25,600 parameters and a 106,231-byte
checkpoint. Exact parentage, config diffs, commands, runtimes, and backtracks are recorded in
`results/backtrack_experiments.csv`; the selected config is `configs/backtrack_best.yaml`.
...
Phase A's configured batch-32 NAFSR step measured 39 seconds on this CPU, projecting roughly
216 hours for 20,000 iterations before periodic validation. CUDA was unavailable and MPS was
unavailable, so neural length/loss/larger-architecture branches exhausted the local compute
budget. The final accepted local-model gain was only +0.0071 dB and TTA added +0.0003 dB;
finer gating regressed and was backtracked. The 29.0 dB target remains 1.5621 dB away.
...
| `train.py` | gradient and LS-5 training paths; `scripts/backtrack_search.py` reproduces the promoted checkpoint |
...
| `configs/` | neural configs, committed validation split, and `backtrack_best.yaml` |
...
| `results/backtrack_experiments.csv` | experiment tree, exact commands, metrics, runtimes, and decisions |
...
| `weights/` | tracked `best.pt` checkpoint + provenance notes |
...
- every number will be labelled with the device it was measured on. The first checkpoint was
  fit on CPU; **no GPU runtime measurement exists and none will be presented as one.**
