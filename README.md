...
`results/baselines/*/metrics.json` (regenerated locally, not committed — `.gitignore` keeps
prediction directories out of the tree).
...
`results/experiments.csv`. That file is **not** in a clone — `.gitignore`'s `results/*` rule
covers it — so both rows are restated above rather than pointed at.
