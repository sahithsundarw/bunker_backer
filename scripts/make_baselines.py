...
#: Proxy-OOD set (U-9, V63): 40 procedurally-generated geometric images, NOT semiconductor
#: imagery (docs/dataset_findings.md "Proxy-OOD set" section; docs/SPEC_ADDENDUM.md section
#: 11 banned-phrase list). Owned/generated by dataset-forensics; this script only reads it.
PROXY_OOD_ROOT = REPO_ROOT / "results" / "eda" / "proxy_ood"
PROXY_OOD_GT = PROXY_OOD_ROOT / "GT"
PROXY_OOD_LR = PROXY_OOD_ROOT / "NoisyLR"
#: Predictions land one level deeper than DEFAULT_OUT_DIR's normal rows (results/baselines/
#: <name>/) specifically so scripts/evaluate.py's `collect_rows` -- which globs
#: `<root>/*/metrics.json`, one level deep -- never picks these up into the main
#: in-distribution table by accident (results/baselines/proxy_ood/<name>/metrics.json is two
#: levels deep under results/baselines).
PROXY_OOD_OUT_SUBDIR = "proxy_ood"

#: Real-SEM OOD set (Round 2 Phase 3, docs/decisions.md D53/D71/D72): real electron-microscopy
#: images, NOT semiconductor imagery specifically (materials-science SEM of a metal matrix
#: composite), used for evaluation only -- no training, no fitting (F17). Owned/generated by
#: scripts/gen_real_sem_ood.py; this script only reads it. --real_sem_ood mirrors --proxy_ood
#: exactly and closes a real reproducibility gap: results/baselines/real_sem_ood/final/ (the
#: original n=45 predictions) previously had no committed generator invocation. The GT/LR
#: source directory and the output subdirectory are both overridable so a larger crop-set
#: (e.g. the n=180 four-crops-per-tile expansion) can be scored without ever touching the
#: original n=45 predictions.
REAL_SEM_OOD_ROOT = REPO_ROOT / "results" / "eda" / "real_sem_ood"
REAL_SEM_OOD_GT = REAL_SEM_OOD_ROOT / "GT"
REAL_SEM_OOD_LR = REAL_SEM_OOD_ROOT / "NoisyLR"
REAL_SEM_OOD_OUT_SUBDIR = "real_sem_ood"
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--baselines", default=None,
                    help=f"comma-separated subset of: {', '.join(ALL_BASELINES)} "
                         "(default: all, or just 'final' with --proxy_ood)")
    ...
    ap.add_argument("--proxy_ood", action="store_true",
                    help="score against results/eda/proxy_ood (40 procedural geometric images, "
                         "U-9/V63) instead of the in-distribution train/ split. Ignores "
                         "--data_root and --split; --data_root/--split need not point anywhere "
                         "for this mode. Predictions land under <out>/proxy_ood/<name>/ so they "
                         "are never picked up by the in-distribution table (see "
                         "PROXY_OOD_OUT_SUBDIR).")
    ap.add_argument("--real_sem_ood", action="store_true",
                    help="score against a real-SEM OOD set (default results/eda/real_sem_ood/, "
                         "Round 2 Phase 3, docs/decisions.md D53/D71/D72) instead of the "
                         "in-distribution train/ split. Ignores --data_root and --split. "
                         "Override the source directory with --real_sem_ood_gt_dir/"
                         "--real_sem_ood_lr_dir (e.g. to point at a larger crop-set such as an "
                         "n=180 four-crops-per-tile expansion) and the output subdirectory with "
                         "--real_sem_ood_out_subdir so predictions for a different set never "
                         "land on top of the original n=45 predictions.")
    ap.add_argument("--real_sem_ood_gt_dir", default=None,
                    help=f"override (default {REAL_SEM_OOD_GT})")
    ap.add_argument("--real_sem_ood_lr_dir", default=None,
                    help=f"override (default {REAL_SEM_OOD_LR})")
    ap.add_argument("--real_sem_ood_out_subdir", default=REAL_SEM_OOD_OUT_SUBDIR,
                    help="output subdirectory under --out for --real_sem_ood predictions "
                         f"(default {REAL_SEM_OOD_OUT_SUBDIR!r}; predictions land at "
                         "<out>/<this>/<name>/)")
    ap.add_argument("--verbose", action="store_true")
    args = ap.parse_args(argv)

    if args.proxy_ood and args.real_sem_ood:
        raise SystemExit("--proxy_ood and --real_sem_ood are mutually exclusive")

    if args.proxy_ood:
        gt_dir, lr_dir = PROXY_OOD_GT, PROXY_OOD_LR
        if not gt_dir.is_dir() or not lr_dir.is_dir():
            raise SystemExit(f"--proxy_ood requires {gt_dir} and {lr_dir} to exist; generate "
                             "the proxy-OOD set first (docs/dataset_findings.md 'Proxy-OOD set')")
        names = sorted(p.name for p in lr_dir.glob("*.npy"))
        gt_names = {p.name for p in gt_dir.glob("*.npy")}
        missing = [n for n in names if n not in gt_names]
        if missing:
            raise SystemExit(f"{len(missing)} proxy-OOD LR file(s) have no GT counterpart, "
                             f"first: {missing[:3]}")
        split_desc = (f"PROXY-OOD: {len(names)} procedurally-generated geometric images "
                      f"({PROXY_OOD_ROOT.relative_to(REPO_ROOT).as_posix()}) -- NOT "
                      "semiconductor imagery, NOT the official test set. Degraded with the "
                      "already-fitted src.degrade.degrade_fitted, zero refitting.")
        root = None
    elif args.real_sem_ood:
        gt_dir = Path(args.real_sem_ood_gt_dir) if args.real_sem_ood_gt_dir else REAL_SEM_OOD_GT
        lr_dir = Path(args.real_sem_ood_lr_dir) if args.real_sem_ood_lr_dir else REAL_SEM_OOD_LR
        gt_dir = gt_dir if gt_dir.is_absolute() else REPO_ROOT / gt_dir
        lr_dir = lr_dir if lr_dir.is_absolute() else REPO_ROOT / lr_dir
        if not gt_dir.is_dir() or not lr_dir.is_dir():
            raise SystemExit(f"--real_sem_ood requires {gt_dir} and {lr_dir} to exist; generate "
                             "a real-SEM OOD set first (scripts/gen_real_sem_ood.py, "
                             "docs/decisions.md D53)")
        names = sorted(p.name for p in lr_dir.glob("*.npy"))
        gt_names = {p.name for p in gt_dir.glob("*.npy")}
        missing = [n for n in names if n not in gt_names]
        if missing:
            raise SystemExit(f"{len(missing)} real-SEM OOD LR file(s) have no GT counterpart, "
                             f"first: {missing[:3]}")
        split_desc = (f"REAL-SEM OOD: {len(names)} real electron-microscopy crops "
                      f"({gt_dir.parent.as_posix()}) -- NOT semiconductor imagery specifically, "
                      "NOT the official test set. Degraded with the already-fitted "
                      "src.degrade.degrade_fitted, zero refitting.")
        root = None
    else:
    ...
    if args.baselines:
        ...
        requested = ["final"] if (args.proxy_ood or args.real_sem_ood) else list(ALL_BASELINES)
    ...
    if args.proxy_ood:
        out_root = out_root / PROXY_OOD_OUT_SUBDIR
    elif args.real_sem_ood:
        out_root = out_root / args.real_sem_ood_out_subdir

    print(f"dataset root : {root if root is not None else '(OOD mode, no train/ root used)'}")
    ...
    print("NOTE         : no test_GT exists; every number is on a held-out slice of train/, "
          "except the proxy-OOD rows which are scored against procedural synthetic GT.")
    print()
    ...
    if args.proxy_ood:
        print("  py -3.12 scripts/evaluate.py --proxy_ood "
              + " ".join(f"--preds {r['name']}={out_root.relative_to(REPO_ROOT).as_posix()}/{r['name']}"
                         for r in ok))
    elif args.real_sem_ood:
        print("  py -3.12 scripts/evaluate.py --real_sem_ood "
              f"--real_sem_ood_gt_dir {gt_dir.relative_to(REPO_ROOT).as_posix() if gt_dir.is_relative_to(REPO_ROOT) else gt_dir} "
              + " ".join(f"--real_sem_ood_pred_dir {out_root.relative_to(REPO_ROOT).as_posix()}/{r['name']}"
                         for r in ok))
    else:
