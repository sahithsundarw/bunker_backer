...
| **V65** | 1 | **Real 256→512 batch correctness, and the OOM-halving recovery path genuinely forced, not faked.** KLA's brief expects eval images around 256×256 or 512×512; the released data is 128→256 only, so 256→512 was previously exercised only by V61's single-pixel shape fixture, and `inference.py`'s recursive OOM-batch-halving (with a CPU-bicubic single-image floor) was exercised by no check at all. | Part A: a real 8-image batch of synthetic 256×256 inputs run through the actual `inference.py` CLI end to end must produce 8 outputs, each `float32`, `ndim==2`, exactly `(512,512)`, finite, within `[0,1]`. Part B, isolated in a child process so it cannot affect any other check: `torch.cuda.set_per_process_memory_fraction()` caps that child process's real CUDA memory low enough that a 16-image batch genuinely exhausts it; `infer_chunk`'s real `_is_oom`-gated except branch must fire at least once (counted via a transparent wrapper, never bypassed or faked), and the final recovered output must still be well-formed (`float32`, `(16,512,512)`, finite, `[0,1]`). SKIP only if no CUDA device is present. |
