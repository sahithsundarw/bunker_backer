...
| **V64** | 1 | **A staged promotion failure must exit non-zero and restore the prior complete output set.** V07 requires exactly one output per input; neither a short new set nor destruction of a previous valid set may be reported as success (adversarial review H4; BUG-010/011). | Using the `mixed` fixture (>= 2 valid inputs), seed one prior `.npy`, then pre-occupy the second expected output path with a directory. The first staged file promotes and the second fails. Inference must exit non-zero, remove every partially promoted file, and restore the prior `.npy` byte-for-byte. |
