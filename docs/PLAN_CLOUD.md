...
| 2026-08-17 | **User-authorised Phase 3** fine-tune (`6a83212acd3824960fcbb566`), `configs/finetune_structural_content.yaml`, resumed from `weights/best.pt`, dispatched via `scripts/dispatch_finetune_structural.py --watch` (in-process manual poll+cancel at 3h -- D67's `timeout` kwarg lesson applied, not repeated) | a100-large | up to 3h (watcher-enforced cap) | up to $7.50 | **up to ~$33.42 if the cap is hit** |

**Explicit user sign-off obtained for this spend** (AskUserQuestion, 2026-08-17) after being shown the ~$25.92 running total was already past the $24 stop-and-report threshold. This run could push the total to ~$33.42 in the worst case, **exceeding the original ~$30 org credit** — the user was told this before authorising. If the watcher's cancel fires as designed, actual cost will be close to but not exceeding the 3h cap; the measured duration (not the cap) will be recorded here once the job's terminal status is checked. **No further cloud spend past this run without re-checking the actual account balance and getting separate sign-off.**

**STOP-AND-REPORT THRESHOLD ($24.00) HAS BEEN EXCEEDED — running total is ~$25.92 before this run.** Flagged
